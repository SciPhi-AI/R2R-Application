...
const Index: React.FC = () => {
  ...
  // Track first render so we can apply smaller left-padding initially:
  ...
  const fetchAllDocuments = useCallback(async () => {
    try {
      ...
      // Once we’ve kicked off our initial fetch, we no longer consider this the "first render."
      // (i.e. we won't switch back and forth even if `loading` changes again later.)
      if (isFirstRender) {
  ...
  }, [pipeline?.deploymentUrl, getClient, isFirstRender]);
  ...
  return (
    <Layout pageTitle="Documents" includeFooter={false}>
      <main className="w-full flex flex-col container h-screen-[calc(100%-4rem)]">
        {/*
          We conditionally apply pl-2 on the very first render,
          and pl-10 thereafter, with a CSS transition so it doesn’t look jarring.
        */}
        <div
          className={
            `relative flex-grow bg-zinc-900 ` +
